 ...
 *  - `priorityKey`: when set, choose the in-range target with the HIGHEST (or
 *    lowest) numeric `entity.state[priorityKey]`, distance as tiebreak, instead of
 *    the plain nearest — so a tower can avoid "shoots the wrong creep": pair with
 *    `follow-path`'s `__pathProgress` for canonical "first" (most-advanced)
 *    targeting, or a creep `hp` state key for "strongest"/"weakest". Unset ⇒ nearest.
 *  - `priorityOrder`: `"high"` (default, prefer the largest value) | `"low"`.
 */
export const aiAimAndFire: BehaviorFn = (entity, world, params, _dt, scratch) => {
  ...
  // Target selection: by default the NEAREST tagged entity. With a
